...
| **Chip bg**          | `#F0F0F0` | Service chip pozadina, total hours pill         |
| **Senior avatar bg** | `#E0F5F5` | Krug iza senior person ikone u review karticama |
...
### 5.13 Service Chip (job detail + schedule)

- Text-only (no icons inside chip)
- Background: `#F0F0F0`
- Text: `#757575`, fontSize implicit (bodySmall)
- Padding: horizontal 12, vertical 2
- Border radius: 12
- Displayed in `Wrap` widget with `work_outline` icon on the left (grey, 20px)
- `crossAxisAlignment: CrossAxisAlignment.start` — icon aligns with first row

### 5.14 Bar Chart (statistics)

- Pure Flutter (Container-based, no external packages)
- Bar color (with data): teal `#009D9D`, borderRadius 6
- Bar color (no data): `#F0F0F0`
- Min height: 4px, max height: 100px (proportional to max value)
- Label above bar: hours value (e.g. "3h"), fontSize 10, `#757575`
- Label below bar: day letter or date range, fontSize 9-11, `#757575`
- Bars inside `SizedBox(height: 140)` with `Row` + `Expanded` per bar

### 5.15 Review Card

- White card, border `#E0E0E0`, borderRadius 16
- Senior avatar: 36px circle, `#E0F5F5` bg, `person_outline` icon (teal, 20px)
- Name: bodyMedium, w600; Date: bodySmall, `#757575` (aligned right)
- Stars: 20px, `#FFC107` (filled=star, empty=star_border)
- Comment: bodySmall, below stars
- Padding: 16 all around, bottom margin: 12
